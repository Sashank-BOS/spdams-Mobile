...
class Helper {
    ...
    isEmailValid(email) {
       ...
       return email.length > 0 && validEmailRegex.test(email.toLowerCase()) ? false : true;
    }

    isPasswordValid(password) {
        let validPassRegex = /.{6,20}/;
        return password.length > 0 && validPassRegex.test(password.trim()) ? false : true;
            ...
            personalizations: [
                {
                    ...
                    subject: "BOS: Forgot password request"
                }
            ],
            from: {
                email: "support@bos.com",
                name: "BOS Support"
            },
            content: [
                {
                    type: "text/html",
                    value:
                        '<!DOCTYPE html><html><head><title></title><meta charset="utf-8" /><link href="https://fonts.googleapis.com/css?family=Roboto" rel="stylesheet"></head>' +
